...
action :add do
  begin
    ...
    template "/etc/sysconfig/druid_overlord" do
      ...
      mode 0644
      retries 2
      variables(:heap_overlord_memory_kb => memory_kb, :rmi_address => rmi_address, :rmi_port => rmi_port, :parent_config_dir => parent_config_dir)
      notifies :restart, 'service[druid-overlord]', :delayed
    end
...
action :register do
  begin
    if !node["druid"]["overlord"]["registered"]
      ...
      node.default["druid"]["overlord"]["registered"] = true
...
action :deregister do
  begin
    if node["druid"]["overlord"]["registered"]
      ...
      node.default["druid"]["overlord"]["registered"] = false
