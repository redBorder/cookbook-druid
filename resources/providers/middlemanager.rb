...
action :add do
  begin
    ...
    memory_kb -= heap_middlemanager_memory_kb
    ...
    if worker_capacity.nil?
      worker_capacity = [[(memory_kb / total_memory_per_task_kb), cpu_num * 2].min, 1].max.to_i
    end
    ...
    template "#{config_dir}/runtime.properties" do
      ...
      variables(name: name, cdomain: cdomain, port: port, task_log_dir: task_log_dir,
                ...
                s3_secret_key: s3_secret_key, heap_memory_peon_kb: heap_memory_peon_kb,
                max_direct_memory_peon_kb: max_direct_memory_peon_kb,
    ...
    template '/etc/sysconfig/druid_middleManager' do
      ...
      mode '0644'
      retries 2
      variables(heap_middlemanager_memory_kb: heap_middlemanager_memory_kb,
...
action :remove do
  begin
    ...
    # dir_list = [config_dir, task_log_dir, log_dir, indexing_dir]
...
action :register do
  begin
    ...
    unless node['druid']['middlemanager']['registered']
      query = {}
      ...
      node.normal['druid']['middlemanager']['registered'] = true
...
action :deregister do
  begin
    if node['druid']['middlemanager']['registered']
      ...
      node.normal['druid']['middlemanager']['registered'] = false
      Chef::Log.info('Druid middlemanager service has been deregistered to consul')
    end
  rescue => e
    Chef::Log.error(e.message)
